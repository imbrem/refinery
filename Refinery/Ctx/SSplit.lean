...
inductive Var?.SSplit : Var? α ε → Var? α ε → Var? α ε → Type _
  | left (v) : SSplit v v v.erase
  | right (v) : SSplit v v.erase v
  | sboth {v} : v.scopy → SSplit v v v

theorem Var?.SSplit.left_unused {u v w : Var? α ε} (σ : u.SSplit v w) (h : u.unused) : v.unused
  := by cases σ <;> first | assumption | rfl

theorem Var?.SSplit.right_unused {u v w : Var? α ε} (σ : u.SSplit v w) (h : u.unused) : w.unused
  := by cases σ <;> first | rfl | assumption

theorem Var?.SSplit.used_of_left {u v w : Var? α ε} (σ : u.SSplit v w) (h : v.used) : u.used
  := by cases σ <;> first | assumption | cases h

theorem Var?.SSplit.used_of_right {u v w : Var? α ε} (σ : u.SSplit v w) (h : w.used) : u.used
  := by cases σ <;> first | cases h | assumption

theorem Var?.SSplit.erase_eq_left {u v w : Var? α ε} (h : u.SSplit v w)
  : u.erase = v.erase := by cases h <;> simp [*]

theorem Var?.SSplit.erase_eq_right {u v w : Var? α ε} (h : u.SSplit v w)
  : u.erase = w.erase := by cases h <;> simp [*]

theorem Var?.SSplit.erase_eq_both {u v w : Var? α ε} (h : u.SSplit v w)
  : v.erase = w.erase := by cases h <;> simp [*]

def Var?.SSplit.both (v : Var? α ε) [h : IsRel v] : SSplit v v v := if hv : v.used then
...
def Var?.SSplit.wkLeft {u v w : Var? α ε} (u' : Var? α ε)
  : u.SSplit v w → Var? α ε
  | .left _ => if u.used then u' else u'.erase
  | .right _ => u'.erase
  | .sboth _ => if u.used then u' else u'.erase

@[simp]
theorem Var?.SSplit.wkLeft_left {u' u : Var? α ε}
  : (SSplit.left u).wkLeft u' = if u.used then u' else u'.erase := rfl

@[simp]
theorem Var?.SSplit.wkLeft_right {u' u : Var? α ε}
  : (SSplit.right u).wkLeft u' = u'.erase := rfl

@[simp]
theorem Var?.SSplit.wkLeft_sboth {u' u : Var? α ε} (h : u.scopy)
  : (SSplit.sboth h).wkLeft u' = if u.used then u' else u'.erase := rfl

def Var?.SSplit.wkRight {u v w : Var? α ε} (u' : Var? α ε)
  : u.SSplit v w → Var? α ε
...
@[simp]
theorem Var?.SSplit.wkRight_left {u' u : Var? α ε}
  : (SSplit.left u).wkRight u' = if u.used then u'.erase else u' := rfl

@[simp]
theorem Var?.SSplit.wkRight_right {u' u : Var? α ε}
  : (SSplit.right u).wkRight u' = u' := rfl

@[simp]
theorem Var?.SSplit.wkRight_sboth {u' u : Var? α ε} (h : u.scopy)
...
theorem Var?.SSplit.left_ty_eq {u v w : Var? α ε} (h : u.SSplit v w) : u.ty = v.ty
  := by cases h <;> rfl

theorem Var?.SSplit.right_ty_eq {u v w : Var? α ε} (h : u.SSplit v w) : u.ty = w.ty
  := by cases h <;> rfl

theorem Var?.SSplit.left_eff_eq {u v w : Var? α ε} (h : u.SSplit v w) : u.eff = v.eff
  := by cases h <;> rfl

theorem Var?.SSplit.right_eff_eq {u v w : Var? α ε} (h : u.SSplit v w) : u.eff = w.eff
  := by cases h <;> rfl

theorem Var?.SSplit.left_eq_erase {u v w : Var? α ε} (h : u.SSplit v w) (hv : v.unused)
  : v = u.erase := by cases u; cases h <;> cases hv <;> rfl

theorem Var?.SSplit.right_eq_of_left {u v w : Var? α ε} (h : u.SSplit v w) (hv : v.unused)
  : w = u := by cases u; cases h <;> cases hv <;> rfl

theorem Var?.SSplit.right_eq_erase {u v w : Var? α ε} (h : u.SSplit v w) (hw : w.unused)
  : w = u.erase := by cases u; cases h <;> cases hw <;> rfl

theorem Var?.SSplit.left_eq {u v w : Var? α ε} (h : u.SSplit v w) (hv : v.used)
  : v = u := by cases h <;> first | rfl | cases hv

theorem Var?.SSplit.right_eq {u v w : Var? α ε} (h : u.SSplit v w) (hw : w.used)
  : w = u := by cases h <;> first | rfl | cases hw

theorem Var?.SSplit.scopy {u v w : Var? α ε}
  (h : u.SSplit v w) (hv : v.used) (hw : w.used) : u.scopy
  := by cases h <;> first | cases hv | cases hw | assumption

def Var?.SSplit.cast {u v w u' v' w' : Var? α ε}
  (h : u.SSplit v w) (hu : u = u') (hv : v = v') (hw : w = w')
  : u'.SSplit v' w' := hu ▸ hv ▸ hw ▸ h

def Var?.SSplit.choose {u v w : Var? α ε} (h : Nonempty (u.SSplit v w)) : u.SSplit v w
  := if hv : v.used then
...
def Ctx?.SSplit.choose {Γ Δ Ξ : Ctx? α ε} (h : Nonempty (Ctx?.SSplit Γ Δ Ξ)) : Ctx?.SSplit Γ Δ Ξ
  := match Γ, Δ, Ξ, h with
  | .nil, .nil, .nil, h => .nil
...
theorem Ctx?.SSplit.left_length {Γ Δ Ξ : Ctx? α ε} (h : Ctx?.SSplit Γ Δ Ξ)
  : Ctx?.length Γ = Ctx?.length Δ
  := by induction h <;> simp [*]

theorem Ctx?.SSplit.right_length {Γ Δ Ξ : Ctx? α ε} (h : Ctx?.SSplit Γ Δ Ξ)
  : Ctx?.length Γ = Ctx?.length Ξ
  := by induction h <;> simp [*]

theorem Ctx?.SSplit.target_length {Γ Δ Ξ : Ctx? α ε} (h : Ctx?.SSplit Γ Δ Ξ)
  : Ctx?.length Δ = Ctx?.length Ξ := h.left_length.symm.trans h.right_length

def Ctx?.both (Γ : Ctx? α ε) [hΓ : Γ.copy] : Γ.SSplit Γ Γ := (λ_ => match Γ with
  | .nil => .nil
  | .cons Γ v => .cons (have _ := copy.tail Γ v; Γ.both) (have _ := copy.head Γ v; .both v)) hΓ

def Var?.SSplit.comm {u v w : Var? α ε} : u.SSplit v w → u.SSplit w v
  | .left _ => .right _
  | .right _ => .left _
  | .sboth h => .sboth h

def Var?.SSplit.v12_3_23 {u₁₂₃ u₁₂ u₁ u₂ u₃ : Var? α ε}
  : u₁₂₃.SSplit u₁₂ u₃ → u₁₂.SSplit u₁ u₂ → Var? α ε
  | .left _, .left _ => u₁₂₃.erase
  | _, _ => u₁₂₃

def Var?.SSplit.s12_3_1_23 {u₁₂₃ u₁₂ u₁ u₂ u₃ : Var? α ε}
...
def Var?.SSplit.s12_3_23 {u₁₂₃ u₁₂ u₁ u₂ u₃ : Var? α ε}
...
def Var?.SSplit.v1_23_12 {u₁₂₃ u₂₃ u₁ u₂ u₃ : Var? α ε}
  : u₁₂₃.SSplit u₁ u₂₃ → u₂₃.SSplit u₂ u₃ → Var? α ε
  | .right _, .right _ => u₁₂₃.erase
  | _, _ => u₁₂₃

def Var?.SSplit.s1_23_12_3 {u₁₂₃ u₂₃ u₁ u₂ u₃ : Var? α ε}
...
def Var?.SSplit.s1_23_12 {u₁₂₃ u₂₃ u₁ u₂ u₃ : Var? α ε}
...
def Ctx?.SSplit.comm {Γ Δ Ξ : Ctx? α ε} : Γ.SSplit Δ Ξ → Γ.SSplit Ξ Δ
  | .nil => .nil
  | .cons h hvw => .cons (SSplit.comm h) hvw.comm

def Ctx?.SSplit.c1_23_12 {Γ₁₂₃ Γ₂₃ Γ₁ Γ₂ Γ₃ : Ctx? α ε}
  : Γ₁₂₃.SSplit Γ₁ Γ₂₃ → Γ₂₃.SSplit Γ₂ Γ₃ → Ctx? α ε
  | .nil, .nil => .nil
  | .cons h hvw, .cons h' hvw' => .cons (c1_23_12 h h') (Var?.SSplit.v1_23_12 hvw hvw')

def Ctx?.SSplit.s1_23_12_3 {Γ₁₂₃ Γ₂₃ Γ₁ Γ₂ Γ₃ : Ctx? α ε}
...
def Ctx?.SSplit.s1_23_12 {Γ₁₂₃ Γ₂₃ Γ₁ Γ₂ Γ₃ : Ctx? α ε}
...
def Ctx?.SSplit.c12_3_23 {Γ₁₂₃ Γ₁₂ Γ₁ Γ₂ Γ₃ : Ctx? α ε}
  : Γ₁₂₃.SSplit Γ₁₂ Γ₃ → Γ₁₂.SSplit Γ₁ Γ₂ → Ctx? α ε
  | .nil, .nil => .nil
  | .cons h hvw, .cons h' hvw' => .cons (c12_3_23 h h') (Var?.SSplit.v12_3_23 hvw hvw')

def Ctx?.SSplit.s12_3_1_23 {Γ₁₂₃ Γ₁₂ Γ₁ Γ₂ Γ₃ : Ctx? α ε}
...
def Ctx?.SSplit.s12_3_23 {Γ₁₂₃ Γ₁₂ Γ₁ Γ₂ Γ₃ : Ctx? α ε}
...
def Var?.SSplit.toSplit {u v w : Var? α ε} (h : u.SSplit v w) : u.Split v w := match h with
  ...
  | Var?.SSplit.right _ => Var?.Split.right (le_refl _) _
  | Var?.SSplit.sboth hu => Var?.Split.sboth hu (le_refl _) (le_refl _)

def Ctx?.SSplit.toSplit {Γ Δ Ξ : Ctx? α ε} : Ctx?.SSplit Γ Δ Ξ → Ctx?.Split Γ Δ Ξ
  | .nil => .nil
  | .cons h hvw => .cons h.toSplit hvw.toSplit

def Var?.SSplit.wk {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w) :
...
@[simp]
theorem Var?.SSplit.wk_left_quant {u' : Var? α ε} {A : Ty α} {q : Quant} {e : ε}
  (ρ : u' ≤ ⟨A, q, e⟩) : (SSplit.left ⟨A, q, e⟩).wk ρ = .left u' := rfl

@[simp]
theorem Var?.SSplit.wk_right_quant {u' : Var? α ε} {A : Ty α} {q : Quant} {e : ε}
  (ρ : u' ≤ ⟨A, q, e⟩) : (SSplit.right ⟨A, q, e⟩).wk ρ = .right u' := rfl

@[simp]
theorem Var?.SSplit.wk_left_zero {u' : Var? α ε} {A : Ty α} {e : ε}
  (ρ : u' ≤ ⟨A, 0, e⟩) : (SSplit.left ⟨A, 0, e⟩).wk ρ = .right u' := rfl

@[simp]
theorem Var?.SSplit.wk_right_zero {u' : Var? α ε} {A : Ty α} {e : ε}
  (ρ : u' ≤ ⟨A, 0, e⟩) : (SSplit.right ⟨A, 0, e⟩).wk ρ = .right u' := rfl

@[simp]
theorem Var?.SSplit.wk_sboth_quant {u' : Var? α ε} {A : Ty α} {q : Quant} {e : ε}
  (ρ : u' ≤ ⟨A, q, e⟩) (h : (Var?.mk A q e).scopy)
  : (SSplit.sboth h).wk ρ = SSplit.sboth (h.anti ρ) := rfl

@[simp]
theorem Var?.SSplit.leftWk {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w)
  : (σ.wkLeft u') ≤ v := by cases u with | mk A q e =>
    cases q using EQuant.casesZero <;> cases σ <;> first | exact ρ | exact (erase_mono ρ)

@[simp]
theorem Var?.SSplit.rightWk {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w)
...
@[simp]
theorem Var?.SSplit.wk_left_zero' {u' : Var? α ε} {A : Ty α} {e : ε}
  (ρ : u' ≤ ⟨A, 0, e⟩) : (SSplit.left ⟨A, 0, e⟩).wk' ρ = .left u' := rfl

@[simp]
theorem Var?.SSplit.wk_right_zero' {u' : Var? α ε} {A : Ty α} {e : ε}
  (ρ : u' ≤ ⟨A, 0, e⟩) : (SSplit.right ⟨A, 0, e⟩).wk' ρ = .left u' := rfl
...
@[simp]
def Ctx?.SSplit.wkLeft {Γ' Γ Δ Ξ : Ctx? α ε}
  : Γ'.Wk Γ → Γ.SSplit Δ Ξ → Ctx? α ε
  | .nil, _ => .nil
  | .skip (v := v) ρ _, σ => (wkLeft ρ σ).cons v.erase
  | .cons (v := v) ρ _, .cons σ hlr => (wkLeft ρ σ).cons (hlr.wkLeft v)

@[simp]
def Ctx?.SSplit.wkRight {Γ' Γ Δ Ξ : Ctx? α ε}
  : Γ'.Wk Γ → Γ.SSplit Δ Ξ → Ctx? α ε
  | .nil, _ => .nil
...
@[simp]
def Ctx?.SSplit.leftWk {Γ' Γ Δ Ξ : Ctx? α ε}
...
@[simp]
def Ctx?.SSplit.rightWk {Γ' Γ Δ Ξ : Ctx? α ε}
...
@[simp]
theorem Ctx?.SSplit.ix_leftWk {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ)
  : (σ.leftWk ρ).ix = ρ := by
    induction ρ generalizing Δ Ξ <;> cases σ <;> simp [*]

theorem Ctx?.SSplit.leftWk_applied {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ) (i : ℕ)
  : (σ.leftWk ρ) i = ρ i := by simp

@[simp]
theorem Ctx?.SSplit.ix_rightWk {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ)
  : (σ.rightWk ρ).ix = ρ := by
    induction ρ generalizing Δ Ξ <;> cases σ <;> simp [*]

theorem Ctx?.SSplit.rightWk_applied {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ) (i : ℕ)
  : (σ.rightWk ρ) i = ρ i := by simp

@[simp]
def Ctx?.SSplit.wk {Γ' Γ Δ Ξ : Ctx? α ε}
...
theorem Var?.SSplit.wkLeft_copy
  {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w) [hv : v.copy]
...
theorem Var?.SSplit.wkLeft_del
  {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w) [hv : v.del]
...
instance Ctx?.SSplit.wkLeft_del
  {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ) [hΔ : Δ.del] : (σ.wkLeft ρ).del
...
instance Ctx?.SSplit.wkLeft_copy
  {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ) [hΔ : Δ.copy] : (σ.wkLeft ρ).copy
  := by induction ρ generalizing Δ Ξ with
  | nil => simp
  | skip ρ hv I => simp [*]
  | cons ρ hvw I =>
...
theorem Var?.SSplit.wkRight_copy'
  {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w) [hw : w.copy]
...
instance Ctx?.SSplit.wkRight_copy'
  {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ) [hΞ : Ξ.copy] : (σ.wkRight' ρ).copy
  := by induction ρ generalizing Δ Ξ with
  | nil => simp
  | skip ρ hv I => simp[*]
  | cons ρ hvw I =>
...
def Ctx?.erase_right : (Γ : Ctx? α ε) → Γ.SSplit Γ Γ.erase
  | .nil => .nil
  | .cons Γ v => .cons Γ.erase_right (.left v)

def Ctx?.erase_left : (Γ : Ctx? α ε) → Γ.SSplit Γ.erase Γ
  | .nil => .nil
  | .cons Γ v => .cons Γ.erase_left (.right v)

theorem Var?.SSplit.wkLeft_quant
  {u' u v w : Var? α ε} (ρ : u' ≤ u) (σ : u.SSplit v w)
...
theorem Ctx?.SSplit.wkLeft_quant
  {Γ' Γ Δ Ξ : Ctx? α ε} (ρ : Γ'.Wk Γ) (σ : Γ.SSplit Δ Ξ)
...
@[simp]
def Var?.SSplit.leftCtx {u v w : Var? α ε} : u.SSplit v w → Ctx? α ε → Ctx? α ε
  | .left _, Γ | .sboth _, Γ => Γ
  | .right _, Γ => Γ.erase

@[simp]
def Var?.SSplit.rightCtx {u v w : Var? α ε} : u.SSplit v w → Ctx? α ε → Ctx? α ε
  | .left _, Γ => Γ.erase
  | .right _, Γ | .sboth _, Γ => Γ

def Var?.SSplit.lift {u v w : Var? α ε} (Γ : Ctx? α ε) (hΓ : quant u ≤ quant Γ)
