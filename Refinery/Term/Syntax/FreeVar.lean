...
theorem eqOn_fvi {a : Term φ α} (σ σ' : Subst φ α) (h : ∀n < a.fvi, σ.get n = σ'.get n)
  : a.subst σ = a.subst σ' := by induction a generalizing σ σ' with
  | bv => apply h; simp [fvi]
  | _ =>
    simp only [fvi, lt_sup_iff, or_imp, forall_and] at h
    (try casesm* _ ∧ _)
    simp
    repeat first | apply And.intro | apply_assumption | apply Subst.eqOn_pred_eqOn_lift
