...
theorem Deriv.den_substD_neg {eσ ea : ε} (he : eσ ↽ ea)
  {Γ Δ : Ctx? α} (σ : SubstDS φ Γ Δ) [hσ : σ.Neg eσ] {A : Ty α} {a : Term φ (Ty α)}
  (D : Δ ⊢ a : A) [ha : a.HasEff ea] : σ.den ≫ D.den ↞ (D.substD σ).den (C := C)
  := by
  generalize ha = ha
  induction D generalizing Γ with
  | bv => simp [substD, den, SubstDS.den_at_neg_eff eσ]
  | op | inl | inr | abort =>
    cases ha
    simp only [den, substD]; rw [<-Category.assoc]; apply refines_comp; apply_assumption; rfl
  | let₁ hΓ da db Ia Ib =>
    ...
    rename_i Γ Γl Γr A B a b
    simp only [den, substD]
    conv => rhs; rw [<-Category.assoc]
    apply refines_trans
    swap
    apply refines_comp
    apply SubstDS.den_ssplit_neg_eff_right eσ
    rfl
    simp only [Category.assoc]
    apply refines_comp; rfl
    apply refines_trans
    ...
    convert Ib ((σ.substLeft hΓ).lift _) hb
    rw [SubstDS.den_lift]
  | unit => simp [substD, den, SubstDS.den_drop_neg_eff eσ]
  | pair hΓ da db Ia Ib  =>
    cases ha with | pair ha hb =>
    rename_i Γ Γl Γr A B a b
    simp only [den, substD, ltimes]
    ...
    swap
    apply refines_comp
    apply SubstDS.den_ssplit_neg_eff_left eσ
    rfl
    simp only [Category.assoc]
    apply refines_comp; rfl
    apply refines_trans
    swap
    apply refines_comp; rfl
    apply E.eff_right_bwd_assoc he
    rw [<-comp_whiskerRight_assoc, <-PremonoidalCategory.whiskerLeft_comp]
    apply refines_comp
    apply refines_whiskerRight
    apply Ia _ ha
  ...
  | iter hΓ hc hd da db Ia Ib =>
    ...
    apply refines_whiskerLeft
    apply Ia _ ha
    apply (E.left_mover_left_uniform he).left_uniform
    apply HasEff.has_eff
    apply HasEff.has_eff
    simp only [Ty.den, Category.assoc]
    rw [<-comp_whiskerRight_assoc]
    ...
    apply refines_comp
    apply refines_whiskerRight
    apply SubstDS.den_fuse_right_eff eσ
    rfl
    simp only [
      rtimes, comp_whiskerRight, Category.assoc, associator_naturality_left_assoc,
      associator_naturality_middle_assoc
